...
| HC-03 | Each course section must receive one base weekly assignment in the MVP. | `section_code`, assignments | GA output validation |
...
| `PRACTICE` | 5 or 6 periods | `1-5`, `1-6`, `2-6` |
| `INTEGRATED` | 5 or 6 periods | `1-5`, `1-6`, `2-6` |
...
| `evening_weekend_avoidance` | SC-08 | Configurable | Adds cost for an evening, Saturday or Sunday allocation only when the matching lecturer preference does not explicitly allow it. Initial values require calibration with real data. |
...
- Exact soft weights still need supervisor/team confirmation.
- Academic calendar and holiday expansion should be implemented after base timetable generation.
- Schedule segment persistence is not part of GA v0.1.
