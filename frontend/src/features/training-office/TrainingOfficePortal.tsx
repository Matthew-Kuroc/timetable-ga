...
const softWeightFields = [
  ["lecturer_preferences", "Ưu tiên giảng viên", 10],
  ["room_capacity_waste", "Hao phí sức chứa phòng", 1],
  ["large_room_small_class", "Phòng lớn cho lớp nhỏ", 25],
  ["schedule_gaps", "Khoảng trống lịch", 4],
  ["scattered_days", "Phân tán ngày dạy", 8],
  ["consecutive_sessions", "Quá nhiều ca liên tiếp", 6],
  ["evening_weekend_avoidance", "Tối/cuối tuần", 5],
] as const;

function GaPage({ batches, onRun }: { batches: Batch[]; onRun: (run: Run) => void }) {
  const [busy, setBusy] = useState(false); const [error, setError] = useState<string | null>(null);
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const data = new FormData(event.currentTarget); const soft_weights = Object.fromEntries(softWeightFields.map(([key]) => [key, Number(data.get(`weight_${key}`))])); const timeLimit = String(data.get("time_limit_seconds") || "").trim(); setBusy(true); setError(null); try { await onRun(await api.startRun({ batch_code: String(data.get("batch_code")), population_size: Number(data.get("population_size")), generations: Number(data.get("generations")), seed: Number(data.get("seed")), crossover_rate: Number(data.get("crossover_rate")), mutation_rate: Number(data.get("mutation_rate")), elite_count: Number(data.get("elite_count")), tournament_size: Number(data.get("tournament_size")), time_limit_seconds: timeLimit ? Number(timeLimit) : undefined, soft_weights })); } catch (reason) { setError(errorText(reason)); } finally { setBusy(false); } };
  const selectedBatch = batches[0]; return <section className="panel"><h2>Cấu hình Thuật toán Di truyền</h2><p>Các ràng buộc nghiệp vụ luôn do backend kiểm tra trước khi lưu kết quả.</p>{!batches.length && <div className="alert warning">Hãy xác nhận một bộ dữ liệu CSV trước khi chạy GA.</div>}<form className="form-grid" onSubmit={submit}><label>Bộ dữ liệu<select name="batch_code" required disabled={!batches.length}>{batches.map((batch) => <option value={batch.batch_code} key={batch.batch_code}>{batchLabel(batch)} — xác nhận {formatDateTime(batch.confirmed_at)}</option>)}</select><small className="field-help">{selectedBatch ? `${selectedBatch.section_count || 0} lớp · Phiên bản ${selectedBatch.version_number || 1} · Mã: ${selectedBatch.batch_code}` : ""}</small></label><label>Kích thước quần thể<input name="population_size" type="number" min="1" defaultValue="80" required /></label><label>Số thế hệ<input name="generations" type="number" min="1" defaultValue="200" required /></label><label>Seed<input name="seed" type="number" defaultValue="42" required /></label><label>Giới hạn thời gian (giây)<span>(tùy chọn)</span><input name="time_limit_seconds" type="number" min="1" step="1" placeholder="Không giới hạn" /></label><label>Tỷ lệ lai ghép<input name="crossover_rate" type="number" min="0" max="1" step="0.05" defaultValue="0.8" required /></label><label>Tỷ lệ đột biến<input name="mutation_rate" type="number" min="0" max="1" step="0.05" defaultValue="0.1" required /></label><label>Số cá thể tinh hoa<input name="elite_count" type="number" min="0" defaultValue="2" required /></label><label>Kích thước tournament<input name="tournament_size" type="number" min="1" defaultValue="3" required /></label><div className="weight-group"><h3>Trọng số ràng buộc mềm</h3><p className="field-help">Các giá trị được lưu cùng lần chạy để có thể giải thích và tái lập kết quả.</p>{softWeightFields.map(([key, label, value]) => <label key={key}>{label}<input name={`weight_${key}`} type="number" min="0" step="0.1" defaultValue={value} required /></label>)}</div><div className="form-action"><button disabled={busy || !batches.length}>{busy ? "Đang chạy..." : "Chạy thuật toán"}</button></div></form>{error && <div className="alert error">{error}</div>}</section>;
}
