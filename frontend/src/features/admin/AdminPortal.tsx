...
function AccountsPage() {
  ...
  const [provisionAll, setProvisionAll] = useState(false);
  const [provisionCodes, setProvisionCodes] = useState("");
  const [credentials, setCredentials] = useState<Array<{ lecturer_code: string; username: string; temporary_password: string }>>([]);
  ...
  return <>
    <section className="panel"><div className="panel-heading"><div><h2>Cấp tài khoản Giảng viên</h2><p>Tài khoản mới dùng mật khẩu tạm riêng và phải đổi ở lần đăng nhập đầu tiên.</p></div></div><form className="form-grid" onSubmit={async (event) => { event.preventDefault(); setError(null); setNotice(null); try { const result = await api.provisionLecturers({ all_lecturers: provisionAll, lecturer_codes: provisionAll ? [] : provisionCodes.split(/[\s,;]+/).filter(Boolean) }); setCredentials(result.created); setNotice(`Đã tạo ${result.created.length}, bỏ qua ${result.skipped.length}, xung đột ${result.conflicts.length}.`); await load(); } catch (cause) { setError(cause instanceof Error ? cause.message : "Không thể cấp tài khoản."); } }}><label className="checkbox-label"><input type="checkbox" checked={provisionAll} onChange={(event) => setProvisionAll(event.target.checked)} /> Cấp cho toàn bộ giảng viên trong batch mới nhất</label>{!provisionAll && <label>Mã giảng viên (cách nhau bằng dấu phẩy hoặc xuống dòng)<textarea value={provisionCodes} onChange={(event) => setProvisionCodes(event.target.value)} placeholder="GV001, GV002" required /></label>}<button>Cấp tài khoản</button></form>{credentials.length > 0 && <div className="alert warning"><strong>Mật khẩu tạm chỉ hiển thị một lần:</strong><ul>{credentials.map((item) => <li key={item.lecturer_code}>{item.lecturer_code} · {item.username} · <code>{item.temporary_password}</code></li>)}</ul><button type="button" className="secondary" onClick={() => setCredentials([])}>Đã lưu an toàn, ẩn danh sách</button></div>}</section>
    <section className="panel">
      ...
      {loading ? <p className="empty" role="status">Đang tải danh sách tài khoản...</p> : users.length ? <div className="table-wrap"><table><thead><tr><th>Tài khoản</th><th>Vai trò</th><th>Mã giảng viên</th><th>Trạng thái</th><th>Đăng nhập gần nhất</th><th>Thao tác</th></tr></thead><tbody>{users.map((item) => <tr key={item.id}><td><strong>{item.display_name}</strong><small>{item.username}</small>{item.must_change_password && <small className="field-help">Đang chờ đổi mật khẩu</small>}</td><td>{roleLabel(item.role)}{item.system_account && <small>Tài khoản hệ thống</small>}</td><td>{item.lecturer_code || "—"}</td><td><span className={`status ${item.active ? "completed" : "failed"}`}>{item.active ? "Đang hoạt động" : "Đã vô hiệu hóa"}</span></td><td>{formatDateTime(item.last_login_at)}</td><td>{item.system_account ? <span className="field-help">Cố định</span> : <>{item.role === "LECTURER" && <button type="button" className="secondary" onClick={async () => { if (!window.confirm(`Cấp lại mật khẩu tạm cho ${item.username}?`)) return; try { const result = await api.resetLecturerPassword(item.id); setCredentials([{ lecturer_code: item.lecturer_code || "", username: result.user.username, temporary_password: result.temporary_password }]); setNotice("Đã cấp lại mật khẩu tạm. Hãy lưu lại ngay."); await load(); } catch (cause) { setError(cause instanceof Error ? cause.message : "Không thể cấp lại mật khẩu."); } }}>Cấp lại mật khẩu</button>} <button type="button" className="secondary" onClick={() => setEditing(item)}>Chỉnh sửa</button></>}</td></tr>)}</tbody></table></div> : !error && <p className="empty">Không có tài khoản phù hợp với bộ lọc.</p>}
...
function UserForm({ mode, user, lecturerOptions, lecturerBatchCode, lecturerBatchName, onCancel, onSaved }: UserFormProps) {
  ...
  return <form className="account-form" onSubmit={submit}>
    ...
    <div className="form-grid">
      ...
      <label>Họ và tên <span aria-hidden="true">*</span><input value={displayName} onChange={(event) => setDisplayName(event.target.value)} readOnly={role === "LECTURER"} required /></label>
